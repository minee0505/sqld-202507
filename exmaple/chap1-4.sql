...
-- replace 파라미터 2개면 해당하는 글자 찾아서 지워버림
SELECT
...
select
    TO_CHAR(creation_date, 'YYYY"년" MM"월" DD"일" DAY') AS "작성일"
from POSTS
where POST_ID = 4;

select
    username,
    NVL(TO_CHAR(manager_id), '최상위 관리자') AS "NVL 처리"
from USERS
;

select
    CASE
        WHEN TO_CHAR(last_login_date, 'YYYY') >= '2024' THEN '최근 활동 유저'
        ELSE '휴면 유저'
        END AS "활동 상태"
from USERS
;
